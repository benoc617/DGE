...
- **AI:** **`drainDoorGameAiTurns`** (via **`runDoorGameAITurns`**) runs until no AI still owes daily slots (guard cap). The next AI (or batch) is ordered by **fewest `fullTurnsUsedThisRound` first**, then **`turnOrder` ascending** — fair interleaving for simultaneous play. **`runDoorGameAITurns`** wraps the drain with **`doorAiInFlight`** so overlapping kicks from **`tryRollRound`** (post-`day_complete`), a human **`after()`**, and **`GET /api/game/status`** serialize per session. **`runDoorGameAITurns`** runs after a human action (background `after()`) and from **`GET /api/game/status`** when any AI still has daily full turns left (mid-round catch-up). **Optional parallel decide (`DOOR_AI_PARALLEL_DECIDE=1`):** up to **`DOOR_AI_DECIDE_BATCH_MAX`** (default **4**) eligible AIs have **`getAIMoveDecision`** (Gemini / MCTS / fallback) overlapped with **`Promise.all`** after each has **`openFullTurn`** applied **serially**; **`processAction`** / **`closeFullTurn`** still run **one empire at a time** in that same order. Decisions are best-effort against the DB state before the wave’s applies; later applies may invalidate earlier decisions — invalid moves use the existing skip / **`end_turn`** path. **Default (flag unset):** one AI full turn at a time, same ordering. **Concurrency caps (global):** **`GEMINI_MAX_CONCURRENT`** (default **4**) limits concurrent Gemini **`generateContent`** calls; **`DOOR_AI_MAX_CONCURRENT_MCTS`** (default **1**) limits concurrent Optimal-persona MCTS work. Each AI move is capped by a **wall-clock timeout** (`getAIMoveDecision` races a timer; on timeout the AI **end_turn**s that slot). If `processAiMoveOrSkip` takes the **skip** path (invalid action → `end_turn`), **`closeFullTurn` must still run** — same as a direct `end_turn` — or `turnOpen` stays true, `fullTurnsUsedThisRound` never increments, and the round can stall (`applyDoorGameAIMove` pairs skip + success with `closeFullTurn`). Set **`DOOR_AI_LOG_WAVE=1`** or **`SRX_LOG_AI_TIMING=1`** for optional **`doorWave`** JSON lines (parallel path).
