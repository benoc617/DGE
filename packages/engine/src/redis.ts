/**
 * @dge/engine — Singleton ioredis client — fail-open throughout.
 * When REDIS_URL is unset or Redis is unreachable, all helpers return null/undefined
 * and callers fall through to the DB without errors.
 */
import Redis from "ioredis";

type RedisOrNull = Redis | null;

const g = globalThis as unknown as { _dgeRedis?: RedisOrNull };
...
function createClient(): RedisOrNull {
  ...
  try {
    ...
    // Suppress "unhandled error" events so a Redis outage doesn't crash the app.
    client.on("error", () => {});
    return client;
  } catch {
    return null;
...
export function getRedis(): RedisOrNull {
  if (g._dgeRedis === undefined) {
    g._dgeRedis = createClient();
  }
  return g._dgeRedis;
}
