/**
 * @dge/engine — Door-game / Gemini AI concurrency runtime settings.
 *
 * Reads from `SystemSettings` DB row (shared engine schema), falls back to
 * env vars, then hardcoded defaults. Cached ~60s per process; invalidated by
 * admin settings PATCH via `invalidateDoorAiRuntimeCache()`.
 */
