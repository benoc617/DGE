...
- `src/lib/simulation.ts` — orphan-player simulation (no `GameSession`). Ten strategies (`DEFAULT_SIM_STRATEGIES` + `"mcts"` + `"maxn"`), `strategyContextFromEmpire`, `finalizeSimSummaries`, `pickSimAction` (overloaded: accepts optional `PrismaEmpireShape` + `rivalShapes` for search strategies), balance warnings, CSV export.
...
- `src/lib/sim-state.ts` — pure in-memory game state for search algorithms. `PureEmpireState`, `applyTick`, `applyAction`, `generateCandidateMoves`, `evalState`, `makeRng` (local mulberry32 seeded RNG), `cloneEmpire`, `empireFromPrisma`. No async, no DB, no global RNG mutation.
- `src/lib/search-opponent.ts` — N-player MCTS (UCB1 + rollout + backprop) and shallow MaxN search. Configurable via `MCTSConfig` / `MaxNConfig`. Entry points: `mctsSearch`, `maxNMove`, `searchOpponentMove`, `buildSearchStates`.
...
- `src/lib/create-ai-players.ts` — `createAIPlayersForSession`; shared AI names/types in `ai-builtin-config.ts` (6 configs: Admiral Koss/economist, Warlord Vrex/warlord, Shadow Nyx/spymaster, Ambassador Sol/diplomat, Fortress Prime/turtle, **Optimax/optimal**).
