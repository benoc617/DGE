 ...
 *   --reset           Reset database before simulation
...
Options:
  ...
  --strategies S    Comma-separated strategies
  --reset           Reset DB first
  --repeat N        Run N simulations with incrementing seeds
`);
        process.exit(0);
    }
...
async function main(): Promise<void> {
  ...
  for (let run = 0; run < opts.repeat; run++) {
    ...
    const config: SimConfig = {
      ...
      seed,
      verbosity: opts.repeat > 1 ? Math.min(opts.verbosity, 1) : opts.verbosity,
  ...
  if (opts.repeat > 1) {
    ...
    console.log(`\n  Win rates:`);
    for (const [strat, wins] of Object.entries(stratWins).sort((a, b) => b[1] - a[1])) {
